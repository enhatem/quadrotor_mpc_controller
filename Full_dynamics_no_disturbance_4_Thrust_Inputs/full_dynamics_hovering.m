...
BEGIN_ACADO;
    ...
    DifferentialState X Y Z u v w phi theta psi p q r;         % z and zDot

    Control F1 F2 F3 F4;                                          % Thrusts
    ...
    h = {X, Y, Z, F1 F2 F3 F4};             % The LSQ function

    Q = eye(7);                             % The weighting matrix
    Q(1,1) = 10;
    Q(2,2) = 10;
    Q(3,3) = 10;

    r = zeros(1,7);                         % The reference 
    ...
    ocp.subjectTo( 0 <= F1 <= m*g );        % Bounds on the total thrust
    ocp.subjectTo( 0 <= F2 <= m*g );
    ocp.subjectTo( 0 <= F3 <= m*g );
    ocp.subjectTo( 0 <= F4 <= m*g );
