...
BEGIN_ACADO;
    ...
    f.linkMatlabODE('ode1');                              % Link a Matlab ODE
    

    f2 = acado.DifferentialEquation();                    % Set the differential equation object for Process                                                     

    f2.linkMatlabODE('ode2');                              % Link a Matlab ODE
    ...
    dynamicSystem = acado.DynamicSystem(f2, identity);       % Set up a dynamic system with the 
...
%         6.0       0.00];
   ...
   % process.initializeAlgebraicStates([-0.3, m*g]);
    ...
    algo = acado.RealTimeAlgorithm(ocp, 0.01);         % The class RealTimeAlgorithm serves as a user-interface 
